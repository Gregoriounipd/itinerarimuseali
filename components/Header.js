...
export default function Header({ itinerarioId }) {
  ...
  return (
    <header className={`w-full sticky top-0 z-50 min-h-[100px] backdrop-blur-md transition-all duration-300 ${
      isModelli
      ...
      <div className="relative flex justify-between items-center px-6 py-4">
            ...
            <div className="relative overflow-hidden rounded-lg transition-transform duration-300 group-hover:scale-105">
              <Image
                ...
                className={`cursor-pointer transition-all duration-300 ${
                  isModelli ? 'drop-shadow-lg' : 'dark:invert group-hover:brightness-110'
                }`}
              />
        ...
        <nav className="hidden md:flex items-center gap-8">
          <Link 
            href="/info" 
            className={`font-semibold text-sm uppercase tracking-wider relative group transition-all duration-300 ${
              isModelli 
          ...
          >
            ...
            <div className={`absolute bottom-0 left-0 w-0 h-0.5 transition-all duration-300 group-hover:w-full ${
              isModelli ? 'bg-white' : 'bg-blue-600 dark:bg-blue-400'
            }`} />
        ...
        <button
          ...
          className={`relative p-2 rounded-lg transition-all duration-300 md:hidden ${
            isModelli 
          ...
          <div className="relative w-7 h-7">
            <Menu 
              ...
              className={`absolute inset-0 transition-all duration-300 ${
                menuOpen ? 'opacity-0 rotate-180 scale-50' : 'opacity-100 rotate-0 scale-100'
              }`} 
            />
            <X 
              size={28} 
              className={`absolute inset-0 transition-all duration-300 ${
                menuOpen ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-180 scale-50'
              }`} 
            />
          </div>
        </button>
